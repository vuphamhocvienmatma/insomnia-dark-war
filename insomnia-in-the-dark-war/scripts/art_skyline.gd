...
const HAZE := Color(0.45, 0.36, 0.42, 0.75)
...
func _ready() -> void:
	...
	while x < 1250.0:
		...
		var h: float = rng.randf_range(50.0, 140.0)
		if x + w > -450.0 and x < 450.0:
			if x < 0.0:
				x = -450.0 - w
			else:
				x = 450.0
	...
	for i in 5:
		_leaves.append({"x": rng.randf_range(-1000.0, 1000.0), "y": rng.randf_range(-120.0, 80.0), "fall": rng.randf_range(10.0, 30.0), "drift": rng.randf_range(-10.0, 10.0)})
