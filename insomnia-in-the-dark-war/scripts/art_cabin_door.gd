...
func _draw() -> void:
	...
	var is_open: bool = bool(door_node.get("is_open")) if door_node != null else false
	var level: int = int(door_node.get("reinforce_level")) if door_node != null else 1
	var hp_ratio: float = float(door_node.get("hp")) / maxf(float(door_node.get("max_hp")), 1.0) if door_node != null else 1.0

	# 1. Doorway Frame (Lintel & jambs from y = -52 to 0)
	...
	if is_open:
		# 2. OPEN DOOR STATE (Swung open into perspective)
		# Open doorway inner shadow
		draw_rect(Rect2(-7.5, -51.0, 15.0, 51.0), Color(0.12, 0.08, 0.06, 0.85))
		
		# Swung door leaf in perspective (pointing inwards/left)
		...
		draw_circle(Vector2(-17.0, -25.0), 1.5, BRASS)
		return
	...
	# --- LEVEL 2: REINFORCED IRON STRAPS & Z-BRACE ---
	if level >= 2:
		# Top and Bottom heavy iron straps
	...
	# --- LEVEL 3: ARMORED STEEL PLATING & BARRICADE ---
	if level >= 3:
		# Heavy steel armor plate covering center
	...
	# Damage cracks if door HP is low
	if hp_ratio < 0.65:
